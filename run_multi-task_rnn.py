...
def train():
  ...
  current_labeling_valid_out_file = result_dir + '/labeling.valid.hyp.txt'
  ...
  with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options, log_device_placement=False)) as sess:
    ...
    while model.global_step.eval() < FLAGS.max_training_steps:
      ...
      if current_step % FLAGS.steps_per_checkpoint == 0:
        ...
        '''
        # @todo
        # calculate metrics of intent detection: f1, precision, accuracy, etc.
        # input is 2 lists, cates is the numbers of label numbers, e.g.7
        def calculate_label_metrix(pred, real, cates):
            mat = np.zeros((cates, cates))
            for i in range(cates):
                for j in range(cates):
                    mat[i, j] = np.max(np.count_nonzero(np.logical_and(pred == i, real == j)), 0)
            # print(mat)
            precisions = np.zeros(cates)
            recalls = np.zeros(cates)
            f1s = np.zeros(cates)
            for i in range(cates):
                precisions[i] = (mat[i, i] * 1.) / np.sum(mat[i, :]) if np.sum(mat[i, :]) != 0 else 0
                recalls[i] = (mat[i, i] * 1.) / np.sum(mat[:, i]) if np.sum(mat[:, i]) != 0 else 0
                f1s[i] = 2 * precisions[i] * recalls[i] / (precisions[i] + recalls[i]) if (precisions[i] + recalls[
                    i]) != 0 else 0
            pre = np.mean(precisions)
            rec = np.mean(recalls)
            f1 = 2 * pre * rec / (pre + rec) if (pre + rec) != 0 else 0
            return precisions, recalls, f1s, pre, rec, f1
        '''
        ...
        # Run evals on development/test set and print the accuracy.
            ...
            ref_tag_list = list() 
            hyp_tag_list = list()
            ref_label_list = list() # @todo
            hyp_label_list = list() # @todo
            correct_count = 0
            accuracy = 0.0
            ...
            # print("predict label list is: %s" % hyp_label_list) #@todo
            # print(len(hyp_label_list))
            # print("real label list is: %s" % ref_label_list)
            # print(len(ref_label_list))
            accuracy = float(correct_count)*100/count #@todo
            if task['intent'] == 1:
              '''
              ...
              '''
              if mode == 'Eval':
                  labeling_out_file = current_labeling_valid_out_file #@todo did not use
              elif mode == 'Test':
                  labeling_out_file = current_labeling_test_out_file #@todo I also want to write label f1 into files
              # _, _, _, pre, rec, f1 = calculate_label_metrix(hyp_label_list, ref_label_list, 7) # @todo maybe should be 8
              f1 = f1_score(ref_label_list, hyp_label_list, average='macro')
              print("  %s label-f1-score: %.2f" % (mode, f1*100))
              sys.stdout.flush()
            ...
            if task['tagging'] == 1:
              ...
              tagging_eval_result = conlleval(hyp_tag_list, 
                                              ref_tag_list, 
                                              word_list, 
                                              taging_out_file) # @todo
              # hyp is the input of prediction
              # ref is the input groundtruth
              # but in the conlleval function, the left column is the groundtruth.
              # print(word_list) # @todo
              print("  %s tag-f1-score: %.2f" % (mode, tagging_eval_result['f1']))
              sys.stdout.flush()
            return accuracy, tagging_eval_result #@todo
            
        # valid @todo did not save best dev model? currently not sure.
        ...
        if task['tagging'] == 1 \
          ...
          # @todo, but did not save the current model?
          subprocess.call(['mv', 
